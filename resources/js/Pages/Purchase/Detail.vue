<template>
    ...
    <div class="m-2 mt-20">
        <FwbAlert class="my-2" closable icon type="success" v-if="$page.props.flash.message">{{
            $page.props.flash.message }}
        </FwbAlert>

        <FwbCard class="p-3 min-w-full mb-3">
            <h1 class="text-xl mb-2"> {{ purchase.customer?.name || "Default Customer" }}</h1>
            <div class="px-2 py-1">
                <span>Date: </span>
                <span> {{ date }}</span>
            </div>
        ...
        <FwbCard class="p-3 min-w-full">
            ...
            <div class="flex justify-between px-2 py-1">
                <span>paid: </span>
                <span>MMK <span contenteditable @blur="(e) => { paid = e.target.innerText }">{{ purchase.paid }}</span>
                </span>
            </div>
            <hr class="h-px my-6 bg-gray-200 border-0 dark:bg-gray-700">
            <div class="flex justify-between px-2 py-1">
...
watch(paid, debounce((p) => {
    router.post(route('purchase.update', { purchase: props.purchase, paid: p, paid_only: true, _method: 'PUT' }))
}, 500))
...
const Submit = () => {
    form.transform((data) => ({
        ...data,
        _method: "PUT",
    })).post(route('purchase.update', props.purchase), {
        onSuccess: () => {
